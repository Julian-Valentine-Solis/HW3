...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        if (node == null) {
            return null;
            // No need to check the balance factor here since we have an empty tree
        }

        if (value < node.value) {// if the value is less than node value, enter left subtree, perform recursion.
            node.leftChild = deleteElement(value, node.leftChild);// searching for value

        } else if (value > node.value) {// if the value is greater than node value, enter right subtree, perform
                                        // recursion.
            node.rightChild = deleteElement(value, node.rightChild);// Searching for value
            // Once we get to this point we have located the value for deletion
        } else if (node.leftChild == null && node.rightChild == null) {// node deleted has no children
            return null;// remove the node, the node is null, When leaf node is removed, there’s no
                        // imbalance introduced that would require rotations.
        } else if (node.leftChild == null) {// if the node that we deleted has one child and that child is a right
            // child, then that child takes it's parents place after deletion
            return node.rightChild;// right child node takes place of deleted node

        } else if (node.rightChild == null) {// if the node that we deleted has one child and that child is a left
                                             // child, then that child takes it's parents place after deletion
            return node.leftChild;// left child node takes place of deleted node

        } else {// if the node that we deleted has two children, we will have to fill the
                // parent's spot with the inOrderSuccessor following the parent's deletion
            Node inOrderSuccessor = minValueNode(node.rightChild);// get minimum value from right subtree
            node.value = inOrderSuccessor.value;// in order successor fills in for deleted node
            node.rightChild = deleteElement(inOrderSuccessor.value, node.rightChild);// Remove the in-order successor
                                                                                     // from the right subtree

        }

        // updating height
        node.height = getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild) + 1);// We have to get the max
                                                                                              // of the subtree depths
                                                                                              // to figure out the
                                                                                              // balance factor, so we
        ...
        // Using the height we just calculated, we can now utilize the getBalanceFactor
        // method
        int balanceFactor = getBalanceFactor(node);// Is it balanced?

        // figure out if this node is balanced, if not, figure out what type of rotation
        // to perform
        // So if the balance factor of this node is > 1 then we know that this node's
        // left subtree is
        // not following within our threshold of -1 < x < 1, so
        if (balanceFactor > 1) {// this node's left subtree is unbalanced, we need to find it.
            // Take a look at the left child of this node's balanceFactor
            if (getBalanceFactor(node.leftChild) >= 0) {// if the balance factor of the left child is greater than or
                                                        // equal to 0 then we know that the left child is also left
                                                        // heavy, so well need to perform a Left left rotation:
                node = LLRotation(node);
            } else {// right subtree of left child is heavy, perform Left right rotation
                node = LRRotation(node);
            }
        } else if (balanceFactor < -1) {// And if the balance factor is less than 1, then we have an imbalance in this
                                        // node's right subtree
            if (getBalanceFactor(node.rightChild) <= 0) {// if the right child's balance factor is less than or equal to
                                                         // 0 then we know that the imbalance is occuring in the right
                                                         // subtree of the right child, perform RRRotation
                node = RRRotation(node);
            } else {// otherwise we know that the imbalance is occuring in the right child's left
                    // subtree, perform right left rotation
                node = RLRotation(node);
            }
